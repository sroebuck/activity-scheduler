...
class ActivityTest extends FunSuite {
  ...
  test("Create some activities") {
    val minMaxMappings= Map("Archery"->(2,6),"Trail Biking"->(2,6),"Ropes Course"->(2,6),"Fire Starter"->(2,6),"High Ropes"->(2,6),"Adventure Golf"->(2,6),"Video & Photography"->(2,6),"Baking"->(6,20),"Games Hall"->(5,20),"Football"->(4,20),"Adventure Playground"->(6,20))

    val slot1ActivityNames = "Archery,Trail Biking,Ropes Course,Fire Starter,High Ropes,Adventure Golf,Baking,Games Hall,Football,Adventure Playground".split(',').toSeq
    val slot1Activities = slot1ActivityNames.map(name => Activity(name, minMaxMappings(name)._1, minMaxMappings(name)._2))

    val slot2ActivityNames = "Archery,Trail Biking,Ropes Course,Fire Starter,High Ropes,Adventure Golf,Video & Photography,Baking,Games Hall,Football,Adventure Playground".split(',').toSeq
    val slot2Activities = slot2ActivityNames.map(name => Activity(name, minMaxMappings(name)._1, minMaxMappings(name)._2))

    val slot3ActivityNames = "Archery,Trail Biking,Ropes Course,Fire Starter,High Ropes,Adventure Golf,Video & Photography,Baking,Games Hall,Football,Adventure Playground".split(',').toSeq
    val slot3Activities = slot3ActivityNames.map(name => Activity(name, minMaxMappings(name)._1, minMaxMappings(name)._2))

    val slot4ActivityNames = "Archery,Trail Biking,Ropes Course,Fire Starter,Adventure Golf,Baking,Games Hall,Football,Adventure Playground".split(',').toSeq
